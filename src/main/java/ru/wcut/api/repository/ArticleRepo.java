...
public interface ArticleRepo extends JpaRepository<ArticleEntity, Long> {
    ...
    @Query(value = "SELECT * FROM article WHERE title_id = '1' AND is_published = 1 ORDER BY dt DESC", nativeQuery = true)
    public List<ArticleEntity> findAllMarket();

    @Query(value = "SELECT * FROM article WHERE title_id = '2' AND is_published = 1 ORDER BY dt DESC", nativeQuery = true)
    public List<ArticleEntity> findAllCompany();

    @Query(value = "SELECT * FROM article WHERE title_id = '3' AND is_published = 1 ORDER BY dt DESC", nativeQuery = true)
    public List<ArticleEntity> findAllShow();

    @Query(value = "SELECT * FROM article WHERE title_id = '4' AND is_published = 1 ORDER BY dt DESC", nativeQuery = true)
    public List<ArticleEntity> findAllItem();

    @Query(value = "SELECT * FROM article WHERE title_id = '5' AND is_published = 1 ORDER BY dt DESC", nativeQuery = true)
...
//
    @Query("select new ArticleEntity(a.id, a.title, a.title_id, a.titlelong, a.img_1, a.dt, a.dt_published, a.is_published) from ArticleEntity a where a.is_published=1 and a.title_id = 4 order by a.dt_published desc")
    public List<ArticleEntity> getNewsItemMainPage(Pageable pageable);

    @Query("select new ArticleEntity(a.id, a.title, a.title_id, a.titlelong, a.img_1, a.dt, a.dt_published, a.is_published) from ArticleEntity a where a.is_published=1 and a.title_id = 5 order by a.dt_published desc")
