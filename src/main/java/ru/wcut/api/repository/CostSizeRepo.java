...
//    SELECT p FROM Person p ORDER BY p.firstName ASC, p.lastName DESC
    ...
    @Query("select m from CostSizeEntity m where m.material_id = :material_id and m.is_published = 1 order by m.size asc")
    List<CostSizeEntity> getAllCostSizeByMaterialTypeId(int material_id);
